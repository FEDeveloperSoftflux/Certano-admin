  ...
  return (
    <header className="flex justify-between items-center py-6 px-8">
      <div className="flex items-center">
        <h1 className="text-2xl font-schibsted font-bold text-white">{pageName}</h1>
      </div>
      
      <div className="flex items-center space-x-4">
        {/* Notification Bell */}
        <button className="relative p-2 rounded-full hover:bg-white/5 transition-all">
          <img src="/Dashboard-assets/notification.svg" alt="Notifications" className="w-5 h-5" />
        ...
        <div className="relative">
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-cards-bg rounded-lg shadow-lg py-2 z-10 animate-fadeIn">
              <a href="#profile" className="block px-4 py-2 hover:bg-white/5">Profile</a>
              <a href="#settings" className="block px-4 py-2 hover:bg-white/5">Settings</a>
              <hr className="my-2 border-white/10" />
              <a href="#logout" className="block px-4 py-2 text-red-400 hover:bg-white/5">Logout</a>
            </div>
          )}
        </div>
      </div>
