        ...
        <div className="space-y-4">
            ...
            {selectedCount > 0 && (
              ...
              >
                <div className="relative">
                  <img src={DeleteIcon} alt="Delete" className="w-4 h-4 filter brightness-0" />
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="relative flex-1">
              <select 
                className="bg-[#222] border border-[#333] text-white px-3 py-2 rounded-lg appearance-none pr-8 focus:outline-none focus:ring-1 focus:ring-[#9d3fff] disabled:opacity-50 disabled:cursor-not-allowed w-full text-sm"
            ...
            <Button
              variant="primary"
              className={`px-3 py-2 bg-gradient-primary shadow-sm transform hover:scale-105 transition-all text-sm ${(isProcessing || isLoading) ? 'opacity-50 cursor-not-allowed' : ''}`}
              onClick={() => handleAction(onAddUser)}
              disabled={isProcessing || isLoading}
            >
              <span className="flex items-center text-black font-semibold">
                <img src={AddIcon} alt="Add" className="w-4 h-4 mr-1" />
                Add User
              </span>
            </Button>
          </div>
